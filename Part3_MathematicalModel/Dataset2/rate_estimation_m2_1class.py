...
estimate1 = minimize(chisquare,init_params,method='SLSQP',bounds=bounds).x
# add gain rate to estimate
estimateFinal1 = np.concatenate(([(mean_transcripts - estimate1[-1])*estimate1[0]],estimate1))
...
#################
################# optimization algorithm (Sequential Least Squares Programming (SLSQP) minimization)
estimate_fixed1 = minimize(chisquare_fixed,init_params,method='SLSQP',bounds=bounds).x
# add gain rate to estimate
estimate_fixedFinal1 = np.concatenate(([(mean_transcripts - estimate_fixed1[-1])*estimate_fixed1[0]],estimate_fixed1))
...
plt.plot(np.arange(1,no_pops+1,1),theo_freq_fixed(estimate_fixedFinal1,False),'*',markersize=10, color = "C1")
plt.plot(np.arange(1,no_pops+1,1),theo_freq_spec_coal(estimateFinal1),'v',markersize=10, color="C4")
