...
################# read data ---------- dataset 1
#################
df = pd.read_csv("data_m1.txt", sep=",", header=None, engine='python', names=['col' + str(x) for x in range(no_pops_all+add_info+1) ])

# exclude Zambian population
no_pops = 6
...
# zeros are also counted -> those are the ones unique to the Zambian population!
emp_spec_aux = np.histogram(dnt_presence,bins = no_pops_all)[0]
emp_spec = emp_spec_aux[1:]
...
gain_init = G1*G12/(G1-G12)/2                # division by two to ensure that initial parameter set is reasonable (2 = two classes
...
def chisquare(params):
    ...
    gain1 = max(0,min(20000,(mean_transcripts - params[1]/params[2]-params[3])*params[0]))
    if gain1 < 0:
        result = 10**9
    else:
...
t = Tree("newick_tree_woZambia.txt",format=1)
