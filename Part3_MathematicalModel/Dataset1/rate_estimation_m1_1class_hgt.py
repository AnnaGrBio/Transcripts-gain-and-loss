...
bounds = [(0,20000),(0,1000),(0,1),(0,ess_upp_bound)]
...
def chisquare(params):
    theo_spec = theo_freq_spec_coal(params)
...
def chisquare_fixed(params):
    ...
    # add extra weight on hitting the mean!
    result += 100*(mean_transcripts-theo_mean)**2
    return(result)
...
#################
################# plot
plt.plot(np.arange(1,no_pops+1,1),theo_freq_fixed(estimate_fixed,False),'o',markersize=10)
plt.plot(np.arange(1,no_pops+1,1),theo_freq_spec_coal(estimate),'^',markersize=10, color="black")
