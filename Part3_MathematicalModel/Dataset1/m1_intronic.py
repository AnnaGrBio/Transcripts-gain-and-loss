...
for j in range(no_pops):
    ind = np.concatenate((ind,np.where(df[names[j+1]].eq('Intergenic').values==True)[0],np.where(df[names[j+1]].eq('ReverseGenic').values==True)[0],np.where(df[names[j+1]].eq('ExonLonger').values==True)[0],np.where(df[names[j+1]].eq('NcRNA').values==True)[0],np.where(df[names[j+1]].eq('Pseudogene').values==True)[0]))
