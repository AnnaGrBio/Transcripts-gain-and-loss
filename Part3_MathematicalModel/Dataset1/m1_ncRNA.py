...
################# read data ---------- dataset 1
#################
df = pd.read_csv("data_m1.txt", sep=",", header=None, engine='python', names=['col' + str(x) for x in range(no_pops+add_info+1) ])
...
for j in range(no_pops):
    ind = np.concatenate((ind,np.where(df[names[j+1]].eq('Intergenic').values==True)[0],np.where(df[names[j+1]].eq('Intronic').values==True)[0],np.where(df[names[j+1]].eq('ReverseGenic').values==True)[0],np.where(df[names[j+1]].eq('ExonLonger').values==True)[0],np.where(df[names[j+1]].eq('Pseudogene').values==True)[0]))
...
gain_init = G1*G12/(G1-G12) 
loss_init = G12/(G1-G12)
...
def chisquare(params):
    gain1 = max(0,min(15000,(mean_transcripts - params[-1])*params[0]))
...
estimateFinal = np.concatenate(([(mean_transcripts - estimate[-1])*estimate[0]],estimate))
...
def chisquare_fixed(params):
    ...
    gain1 = (mean_transcripts - params[-1])*params[0]
...
# add gain rate to estimate
estimate_fixedFinal = np.concatenate(([(mean_transcripts - estimate_fixed[-1])*estimate_fixed[0]],estimate_fixed))

#################
################# plot
plt.plot(np.arange(1,no_pops+1,1),theo_freq_fixed(estimate_fixedFinal,False),'o',markersize=10)
#plt.plot(np.arange(1,no_pops+1,1),theo_freq_fixed(estimate_llh_fixed,False),'v',markersize=10,color='C4')
plt.plot(np.arange(1,no_pops+1,1),theo_freq_spec_coal(estimateFinal),'^',markersize=10, color="black")
#plt.plot(np.arange(1,no_pops+1,1),theo_freq_spec_coal(estimate_bhp),'s',markersize=5, color="C1")
